...
mod registry;
mod telemetry;
use auth::{authorize_deregister, authorize_register};
    ...
    loop {
        ...
        match reader.read_line(&mut line).await {
            ...
            Ok(_) => {
                ...
                if let Some(mut response) = process_mcp_message(line.as_bytes(), &state).await {
...
async fn handle_bus_local(method: &str, msg: &serde_json::Value, state: &AppState) -> Vec<u8> {
    ...
    match method {
        ...
        "_bus.register" => {
            ...
            if let Err(reason) = authorize_register(registered_by, namespace, &handler) {
                warn!("rejected _bus.register from {}: {}", registered_by, reason);
                return error_bytes(id, "E_FORBIDDEN", reason);
            }
        ...
        "_bus.deregister" => {
            let registered_by = params
                ...
                .and_then(|v| v.as_str())
                .unwrap_or("unknown");
            if let Err(reason) = authorize_deregister(registered_by) {
                return error_bytes(id, "E_FORBIDDEN", reason);
            }
            let namespace = params
                .get("namespace")
