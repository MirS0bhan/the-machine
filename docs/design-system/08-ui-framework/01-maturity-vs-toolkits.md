...
| Text / fonts | F | F | F | F | **P→F** | HarfBuzz shape + FreeType raster (`compositor/src/text.rs`), Inter + JetBrains Mono; bitmap fallback only if faces missing |
...
| i18n / RTL / IME | F | F | F | F | **—** | HarfBuzz enables shaping; no locale catalogs / bidi policy yet |
