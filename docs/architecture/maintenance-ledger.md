...
|------------|--------|-----|--------|---------|-------|
| verify-fix event-bus empty main.rs | cursor/maintenance-verify-fix-event-bus-empty-017bcca8 | — | pr-open | 2026-08-30 | restore `event-bus/src/main.rs` wiped by merge `605a650`; union inventory + verify-docs conflict markers |
...
|------------|----------|--------|---------|
| 2026-08-30 | verify-fix | event-bus empty main.rs + inventory conflict markers | pr-open |
...
2. **audit** — missing Python integration test (`localmodel.classify_intent`, `localmodel.embed`, `event.register_handler`, `marketplace.install`, `policy.audit`; skip `event.schedule` — PR #187)
3. **audit** — security pass: grant tokens / lambda entrypoint / external.register proxy rules
4. **G13** — remaining installer polish only after cooldown (operator validation merged in #168)
